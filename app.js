...
function createTableBody(titlesTableHeader, titleHeadTable, valueOfObject) {
  ...
  bodyItem = valueOfObject.map((valueItems, index) => {
    ...
    <td><button class='addButton' data-title='${titleHeadTable}'>+</button><button class='removeButton'><img class='deleteIcon' src="./images/eraser-solid.svg" alt="Delete button"><input type='checkbox'></button></td>
    </tr>
    `;
  });
  ...
  if (e.target === enigmaItem || e.target === puzzleMod) {
    if (!hideWrapper.classList.contains("activeTab")) {
      puzzleText.innerHTML =
        '<p class="alertSolve">Solve the puzzle in JavaScript and feel the power</p><p class="solve">"b" + "a" + + "a" + "a"</p>';
